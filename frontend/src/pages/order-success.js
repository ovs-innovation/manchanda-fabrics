        ...
        <div className="max-w-3xl mx-auto bg-white rounded-[24px] shadow-sm border border-[#E6D1CB]/40 overflow-hidden">
          <div className="p-8 text-center border-b border-[#E6D1CB]/40 bg-[#FAF7F5]">
            ...
            <p className="text-sm text-[#3B2A25]/70 max-w-md mx-auto">
              {t("Thank you for shopping with Manchanda Fabrics. Your payment was successful, and we are preparing your order.")}
            </p>
          </div>
            ...
            {order ? (
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                  <div>
                    <h3 className="font-bold text-[#3B2A25] uppercase tracking-wider text-xs mb-3">{t("Order Details")}</h3>
                    <p className="text-[#3B2A25]/70 mb-1"><span className="font-semibold text-[#3B2A25]">{t("Invoice No:")}</span> #{order.invoice || order._id}</p>
                    <p className="text-[#3B2A25]/70 mb-1"><span className="font-semibold text-[#3B2A25]">{t("Date:")}</span> {new Date(order.createdAt || Date.now()).toLocaleDateString()}</p>
                    <p className="text-[#3B2A25]/70"><span className="font-semibold text-[#3B2A25]">{t("Payment:")}</span> {t(order.paymentMethod || "Online Payment")}</p>
                  </div>
                  <div>
                    <h3 className="font-bold text-[#3B2A25] uppercase tracking-wider text-xs mb-3">{t("Shipping Address")}</h3>
                    ...
                    <p className="text-[#3B2A25]/70 mt-1">{t("Phone:")} {order.user_info?.contact}</p>
                  </div>
                </div>

                {/* Items Summary */}
                <div>
                  <h3 className="font-bold text-[#3B2A25] uppercase tracking-wider text-xs mb-4">{t("Items Ordered")}</h3>
                    ...
                    {order.cart?.map((item, idx) => (
                      <div key={idx} className="py-4 flex justify-between items-center text-sm">
                        <div className="flex-1 min-w-0 pr-4">
                          <p className="font-bold text-[#3B2A25] truncate">{t(item.title)}</p>
                          <p className="text-xs text-[#3B2A25]/60 mt-0.5">{t("Quantity:")} {item.quantity}</p>
                        </div>
              ...
              <div className="text-center py-8">
                <p className="text-[#3B2A25]/70 mb-4">{t("No order details found in session. But don't worry, your order is being processed.")}</p>
              </div>
            )}
