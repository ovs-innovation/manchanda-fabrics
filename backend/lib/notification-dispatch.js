...
function buildEmailHtml({ title, description, image, clickAction, shopName }) {
  const storeUrl = process.env.STORE_URL || process.env.FRONTEND_URL || "https://Manchanda Fabrics.com";
...
<body style="font-family:Arial,sans-serif;background:#f8fafc;padding:24px">
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;padding:24px;border:1px solid #e2e8f0">
    <p style="text-align:center;margin:0 0 16px"><img src="${BRAND_LOGO_URL}" alt="${shopName || "manchanda"}" style="height:48px;width:auto" /></p>
    <p style="color:#0f766e;font-weight:bold;margin:0 0 8px">${shopName || "manchanda"}</p>
...
function buildSmsText({ title, description, clickAction }) {
  const storeUrl = process.env.STORE_URL || "https://Manchanda Fabrics.com";
  const link = clickAction || storeUrl;
...
async function sendEmailsToRecipients(recipients, payload) {
  const shopName = payload.shopName || "manchanda";
